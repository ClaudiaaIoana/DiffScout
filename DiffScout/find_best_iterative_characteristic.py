...
def find_iterative_differential_characteristic(start_state,ddt,P, max_depth, comparison_function=operator.eq, max_simulation=3, probability_limit=20, max_sbox_active=3, MSB=True, verbose=False):
    ...
    start_node = Node_iterative(start_state, ddt, P, max_depth, prev_g=1, move_prob=1, depth=0, max_simulation=max_simulation, MSB=MSB)
    ...
    while frontier:
        ...
        # Goal test
        if current_node.depth == max_depth and comparison_function(current_node.state, start_state):
            if verbose:
                print("Goal reached.")
